...
class CEKGPreprocessor:
    def __init__(self, openai_model: Optional[str] = None):
        if not config.OPENAI_API_KEY: raise RuntimeError("OPENAI_API_KEY not set.")
        ...
        if self.event_ontology: print(f"[config] Loaded custom Event Ontology ({len(self.event_ontology)} types).")
        if self.relationship_ontology: print(f"[config] Loaded custom Relationship Ontology ({len(self.relationship_ontology)} types).")

    def _load_ontology(self, filename: str, key: str) -> Optional[List[str]]:
        if os.path.exists(filename):
            try:
                with open(filename, 'r', encoding='utf-8') as f: return json.load(f).get(key)
            except Exception as e: print(f"[warning] Failed to load {filename}: {e}")
        return None
    ...
    def _parse_event_json_data(self, event_data_list, chapter_id, logprobs, enable_confidence_calibration, graph_model="star"):
        all_events = []
        all_produces = []
        ...
        def _get_ent_id(name, type_prefix, evt_id):
            clean = name.strip().lower().replace(' ', '_')
            base = f"{type_prefix}_{clean}"
            if graph_model == 'chain': return utils._make_id(f"{base}_{evt_id}")
            return utils._make_id(base)
        ...
        for i, event_data in enumerate(event_data_list):
            try:
                ...
                event = schemas.CEKEvent(
                    id=utils._make_id("event"), raw_description=event_data.get("raw_description", event_data.get("name", "Untitled")),
                    event_category=event_data.get("event_category", "OTHER"), action_type=event_data.get("event_category", "OTHER"),
                    time_context=event_data.get("time_context"), location_context=event_data.get("location_context"),
                    actors=[], patients=[], why_factors=why_factors_list, chapter=chapter_id, sequence=seq, confidence=confidence, source_quote=event_data.get("quote", "")
                )
                ...
                for actor_name in event_data.get("actors", []):
                    if isinstance(actor_name, str) and actor_name.strip():
                        ...
                        aid = _get_ent_id(name, "agent", event.id)
                ...
                for pat_name in event_data.get("patients", []):
                    if isinstance(pat_name, str) and pat_name.strip():
                        ...
                        pid = _get_ent_id(name, "agent", event.id)
                ...
                for wf in why_factors_list:
                    wid = _get_ent_id(wf[:30], "why", event.id)
            ...
            except Exception as e: print(f"[warning] Event parse error: {e}"); continue
        return all_events, all_produces, entity_occurrences_batch

    async def _process_text_chunk(self, text_chunk, chapter_id, enable_llm_expansion, enable_confidence_calibration, extraction_style, graph_model):
        try:
            data, logprobs = await llm_service.extract_events_from_text(text_chunk, chapter_id, self.openai_model, self.client, enable_llm_expansion, False, extraction_style, self.event_ontology)
            return self._parse_event_json_data(data, chapter_id, logprobs, enable_confidence_calibration, graph_model)
        except Exception as e: print(f"[error] Chunk processing failed: {e}"); return [], [], defaultdict(list)

    async def _process_parallel_batch(self, paragraphs_with_chapter, enable_llm_expansion, enable_confidence_calibration, extraction_style, graph_model):
        all_events, all_produces, entity_occurrences = [], [], defaultdict(list)
        try:
            results = await llm_service.batch_extract_events(paragraphs_with_chapter, self.openai_model, self.client, enable_llm_expansion, False, extraction_style, self.event_ontology)
            for (json_data, logprobs), (para, cid) in zip(results, paragraphs_with_chapter):
                evs, prods, occs = self._parse_event_json_data(json_data, cid, logprobs, enable_confidence_calibration, graph_model)
    ...
    async def run_async(self, text_path, out_json, out_cypher, out_csv_dir, max_chapters, batch_size, causal_window, causal_sample_rate, causal_batch_size, paragraph_chunk_size, extraction_style, graph_model, enable_scene_grouping, enable_semantic_linking, enable_llm_expansion, enable_confidence_calibration):
        ...
        for cid, txt in chapters:
            ...
            if paragraph_chunk_size != 1:
                ...
                for i in range(0, len(paras), csize):
                    ...
                    e, p, o = await self._process_text_chunk(chunk, cid, enable_llm_expansion, enable_confidence_calibration, extraction_style, graph_model)
                ...
                for i in range(0, len(paras), batch_size):
                    ...
                    e, p, o = await self._process_parallel_batch(batch, enable_llm_expansion, enable_confidence_calibration, extraction_style, graph_model)
                    all_events.extend(e); all_produces.extend(p)
                    for k,v in o.items(): entity_occurrences[k].extend(v)

        print(f"[pipeline] Total events: {len(all_events)}")
        all_events = graph_builder.propagate_context_attributes(all_events)
        new_prods, entity_occurrences = graph_builder.propagate_context(all_events, all_produces, entity_occurrences, graph_model=graph_model)
        all_produces.extend(new_prods)

        e2e_links = graph_builder.create_entity_to_event_links(entity_occurrences, all_produces, graph_model=graph_model)
        ...
        exporters.export_csv(out_csv_dir, all_events, all_produces, e2e_links, causal_links, sem_links, scenes, graph_model=graph_model)
